...
If we decide to exclude &, then only 4 comapny names contain a non-alphabetic character, which is only 0.8%.
...
```{r}
setdiff(dat$address[grepl("[0-9]+$", dat$address)], 
        dat$address[grepl("#[0-9]+$", dat$address)])
```

Note:

We treat any trailing number in an address as an apartment number and extract it accordingly. While this heuristic works for entries like “534 Schoenborn St #51,” it can mislabel some edge cases where the final number is part of the street name rather than a unit, for example “79 State Route 35”. Indeed, there are 5 edge cases. Therefore, I restricted extraction to addresses that end with digits but also come with common apartment token "\#".
